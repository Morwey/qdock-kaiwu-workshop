#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Dock one of the GPM molecules (1N2J / 1LRH) on the CIM:
quota mode, 8-bit (precision=8, truncated_precision=8, no spin expansion).
    ...
    python scripts/dock.py all             # both, summary table
...
if __name__ == "__main__":
    ...
    ap.add_argument("pdb", choices=["1N2J", "1LRH", "all"])
    ap.add_argument("--live", action="store_true", help="resubmit to the CIM (needs a license)")
    a = ap.parse_args()
    targets = ["1N2J", "1LRH"] if a.pdb == "all" else [a.pdb]
    print(f"{'PDB':<6}{'spins':>7}{'poses':>7}{'mRMSD/A':>9}")
    for p in targets:
        nv, npose, m = dock(p, live=a.live)
        print(f"{p:<6}{nv:>7}{npose:>7}{m:>9.2f}")
